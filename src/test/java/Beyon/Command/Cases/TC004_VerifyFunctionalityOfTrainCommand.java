...
public class TC004_VerifyFunctionalityOfTrainCommand {
...
// return true if file here
    ...
    @Test(groups = {"Regression"},description = "Execute train  command  ", priority = 6)
    public void execute_TrainCommand() throws Exception {

        commandMethods = new CommandMethods();

        commandMethods.RunContainerAndExTRain(Data_cmd.dockerPath,
                Data_cmd.projectDirPath.replace("${dirPath}", dirPath), projectName + ":" + dockerImageNumber
                , Data_cmd.createStudy,Data_cmd.model663,Data_cmd.templateSimpleRF);
        String expectedResult= "Closed: train";

        String actualResult = Files.readString(Paths.get(dirPath + "/src/main/resources/DataProvider/ActualResults.txt"));
        ...
        commandMethods.file.delete();
    }
    @Test(groups = {"smoke"},description = "Delete The created study folder ", priority = 7)
